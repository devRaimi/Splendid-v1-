...
function loadOrders() {
	try {
		orders = JSON.parse(localStorage.getItem('sp_orders_v2') || '[]');
	} catch (e) {
		orders = [];
	}
}
...
function setOrderType(type) {
	orderType = type;
	document
		.getElementById('type-normal')
		.classList.toggle('selected', type === 'normal');
	document
		.getElementById('type-gift')
		.classList.toggle('selected', type === 'gift');
	document.getElementById('gift-notice').style.display =
		type === 'gift' ? 'flex' : 'none';
}

// ── Products ──────────────────────────────────────────────────────────
function buildProducts() {
	document.getElementById('product-grid').innerHTML = CONFIG.PRODUCTS.map(
		(p) => `
...
function onSizeChange(id, sel) {
	...
	document
		...
		.classList.toggle('has-item', price > 0 && cart[id].qty > 0);
...
function checkFlavourVisibility() {
	...
	document.getElementById('flavour-wrap').style.display = hasPuff
		? 'block'
		: 'none';
}
...
function buildFlavourTags() {
	document.getElementById('flavour-tags').innerHTML = CONFIG.FLAVOURS.map(
		(f, i) =>
...
function buildCampusTags() {
	document.getElementById('campus-tags').innerHTML = CONFIG.CAMPUSES.map(
		(c, i) =>
...
function pickFlavour(el, val) {
	document
		.querySelectorAll('#flavour-tags .tag')
...
function pickCampus(el, val) {
	document
		.querySelectorAll('#campus-tags .tag')
		.forEach((t) => t.classList.remove('selected'));
	el.classList.add('selected');
	selectedCampus = val;
}
...
function populateBankDetails() {
	document.getElementById('bank-acct-display').textContent =
		CONFIG.BANK_ACCOUNT_NUMBER;
	document.getElementById('bank-name-display').innerHTML =
		`<strong>Bank:</strong> ${CONFIG.BANK_NAME}`;
	document.getElementById('bank-holder-display').innerHTML =
		`<strong>Account name:</strong> ${CONFIG.BANK_ACCOUNT_HOLDER}`;
}
...
function goStep(n) {
	if (n === 2) {
		if (cartTotal() === 0) {
			toast('Please select at least one item 🛍', true);
			return;
		}
		// Show correct fields for order type
		document.getElementById('normal-fields').style.display =
			orderType === 'normal' ? 'block' : 'none';
		document.getElementById('gift-fields').style.display =
			orderType === 'gift' ? 'block' : 'none';
		buildSummaryCard();
	}
	document
...
async function submitOrder() {
	if (!receiptFile) {
		toast('Please upload your payment receipt', true);
		return;
	}
	...
	if (orderType === 'normal') {
		...
		if (!phone) {
			toast('Please enter your WhatsApp number', true);
			return;
		}
		if (!location) {
			toast('Please enter your pickup location', true);
			return;
		}
	} else {
		// Anonymous gift
		...
		if (!recipientName) {
			toast("Please enter the recipient's name", true);
			return;
		}
		if (!location) {
			toast('Please enter the delivery location', true);
			return;
		}
		name = 'Anonymous';
		notes = '';
	...
	if (orderType === 'gift') {
		...
		document.getElementById('success-msg').textContent =
			"Your anonymous gift is on the way. We'll confirm on WhatsApp.";
	} else {
		document.getElementById('success-title').textContent =
			'Order received! 🎉';
		document.getElementById('success-msg').textContent =
			"We've got your order and will confirm on WhatsApp shortly.";
	}
...
function trackOrder() {
	...
	const steps = [
		...
		},
		{
			key: 'confirmed',
			label: 'Confirmed',
			sub: "We're preparing your order",
		},
		{
			key: 'ready',
			label: 'Ready for pickup',
			sub: `Head to ${order.location}`,
		},
		...
		const res = await fetch(CONFIG.SHEET_WEBHOOK_URL, {
			...
			body: JSON.stringify({
				...
				items: order.items
					.map(
						(i) =>
							`${getProduct(i.id).emoji} ${getProduct(i.id).name} x${i.qty} (${i.sizeLabel})`,
					)
					.join(' | '),
